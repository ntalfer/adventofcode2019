defmodule Day2Test do
  ...
  test "part1 examples" do
    assert Day2.part1_final_state([1,9,10,3,2,3,11,0,99,30,40,50]) == [3500,9,10,70,2,3,11,0,99,30,40,50]
    assert Day2.part1_final_state([1,0,0,0,99]) == [2,0,0,0,99]
    assert Day2.part1_final_state([2,4,4,5,99,0]) == [2,4,4,5,99,9801]
    assert Day2.part1_final_state([1, 1, 1, 4, 99, 5, 6, 0, 99]) == [30, 1, 1, 4, 2, 5, 6, 0, 99]
  end

  test "part1 challenge" do
    program = @input |> String.split(",") |> Enum.map(&String.to_integer/1)
    assert Day2.part1(program) == 4023471
  end

  test "part2 challenge" do
    program = @input |> String.split(",") |> Enum.map(&String.to_integer/1)
    assert 8051 == Day2.part2(program, 19690720)
  end
end
